...
function renderThumbnailsContentTpl(pokemon, pkTypeName1, pkTypeName2, bg_Color) { // Funktion für die kleine Pokémon-Karte
    // Start des HTML-Returns für das Thumbnail
    ...
    onclick="showPkDialog(${pokemon.id})"> <div class="thumbnail-name-container"> <span class="thumbnail-name"><b class="pkName"> ${pokemon.name.charAt(0).toUpperCase() + pokemon.name.slice(1)}</b> </span> </div> <div class="thumbnail-typeNames-img"> <div class="pk-types"> <span id="typeSlot1" class="pkType-name1">${pkTypeName1.charAt(0).toUpperCase() +
        ...
        pkTypeName2.slice(1)}</span> </div> <img class="thumbnail-images" src="${pokemon.sprites.other.showdown.front_default}" alt="Image${pokemon.name}"> </div> </button>`; // Ende des Buttons
} // Ende der Funktion

function addfoundPkTpl(pkName) { // Funktion für die Suchvorschläge
    return `<option value="${pkName}">${pkName}</option>`; // Gibt ein Option-Element für die Liste zurück
} // Ende der Funktion

function dialogHeaderTpl() { // Funktion für den Kopfbereich des Dialogs
    return /*html*/`<div class="dialog-body" onclick="closeDialogOutsite(event)"> <header id="headline" class="dialog-header" onclick="closeDialogOutsite(event)"> <nav class="nav-dialog"> <h2 class="headline-title-dialog"> <span class="part-1-dialog">Card</span> <span class="divider-dialog"></span> <span class="part-2-dialog">Pokedex</span> </h2> <button class="close-button-dialog" onclick="closeDialog()" type="button">&times;</button> </nav> </header>`; // Ende des Headers
} // Ende der Funktion

function dialogUpperSectionTpl(thisPokemon, pkTypeName1, pkTypeName2, bg_Color, pkcategory) { // Funktion für den oberen Info-Bereich
...
function dialogFooterTpl(thisPokemon) { // Funktion für den Dialog-Footer
    return `</main> <footer class="footer-dialog" id="dialogFooter" onclick="closeDialogOutsite(event)"> <div id="arrowContainer" class="arrow-container" onclick="closeDialogOutsite(event)"> <button id="arrowLeft" class="arrow-button" onclick="clickButtonPrevious(${thisPokemon.id})"
            ...
            type="button">&#10145</button> </div> </footer> </div>`; // Schließt den dialog-body Container
} // Ende der Funktion

function renderSingelMoveTpl(i, movesNames) { // Funktion für eine einzelne Attacken-Zeile
    return ` <tr id="${i}"> <td>${movesNames[i]}</td> </tr> `; // Ende der Zeile
} // Ende der Funktion
